...
window.getBreakpoint = function () {
    ...
    var sortedKeys = Object.keys(window.breakpoints).sort((a, b) => { return window.breakpoints[a] - window.breakpoints[b]; });
...
window.addResizeListener = function (dotNetRef) {
    const updateDotNet = () => {
        ...
        if (!window.breakpoint || newBp != window.breakpoint) {
            ...
            dotNetRef.invokeMethodAsync('BreakpointReached', { width: window.innerWidth, height: window.innerHeight, breakpoint: newBp });
        }       
    };
